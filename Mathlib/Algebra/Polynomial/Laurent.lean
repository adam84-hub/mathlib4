...
def trunc : R[T;T⁻¹] →+ R[X] :=
  (toFinsuppIso R).symm.toAddMonoidHom.comp <| comapDomain (↑) Nat.cast_injective
...
theorem trunc_C_mul_T (n : ℤ) (r : R) : trunc (C r * T n) = ite (0 ≤ n) (monomial n.toNat r) 0 := by
  ...
  · lift n to ℕ using hn
    simp [toFinsupp_monomial, AddMonoidAlgebra.comapDomain_single, -single_eq_C_mul_T]
  · ext a
    have : a ≠ n := by lia
...
inclusion `ℕ ↪ ℤ`. -/
theorem toLaurent_support (f : R[X]) :
    f.toLaurent.coeff.support = f.support.map Nat.castEmbedding := by
  induction hs : f.support using Finset.induction_on generalizing f with
  | empty => simpa using hs
  | insert a s as hf =>
  have : (erase a f).toLaurent.coeff.support = s.map Nat.castEmbedding := by
    refine hf (f.erase a) ?_
    simp only [hs, Finset.erase_eq_of_notMem as, Polynomial.support_erase,
      Finset.erase_insert_eq_erase]
  rw [← monomial_add_erase f a, Finset.map_insert, ← this, map_add, Polynomial.toLaurent_C_mul_T,
    AddMonoidAlgebra.coeff_add, support_add_eq, Finset.insert_eq]
  · congr
    exact support_C_mul_T_of_ne_zero (Polynomial.mem_support_iff.mp (by simp [hs])) _
  · rw [this]
    exact .mono_left (support_C_mul_T _ _) (by simpa)
